...
import { Send, User, Loader2, MessageSquare, MapPin, ExternalLink } from 'lucide-react';
...
export default function Chat({ listingId, receiverId, listingTitle }: ChatProps) {
  ...
  async function shareLocation() {
    if (!listing?.preciseLocation) return;
    ...
    try {
      const res = await fetch('/api/messages', {
        ...
        body: JSON.stringify({ 
          receiverId, 
          listingId, 
          content: "📍 Shared Collection Location",
          location: {
            lat: listing.preciseLocation.lat,
            lng: listing.preciseLocation.lng,
            address: listing.preciseLocation.address,
            label: "Precise Collection Point"
          }
        })
      });
      if (res.ok) {
    ...
    <div className="flex flex-col h-[600px] bg-white dark:bg-zinc-900/50 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-2xl">
                      ...
                      <div className={`max-w-[80%] p-4 rounded-2xl text-sm font-medium ${
                        ...
                        {msg.location ? (
                          <div className="space-y-3">
                            <div className="flex items-center gap-2 p-2 rounded-xl bg-black/10 dark:bg-black/20 border border-white/10">
                              <MapPin className="w-4 h-4 text-emerald-400" />
                              <span className="text-xs font-bold truncate">{msg.location.label}</span>
                            </div>
                            <p className="text-[11px] leading-relaxed opacity-90">{msg.location.address}</p>
                            <Button 
                              size="sm" 
                              onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${msg.location.lat},${msg.location.lng}`, '_blank')}
                              className="w-full bg-white text-emerald-950 hover:bg-zinc-100 h-8 text-[10px] font-black uppercase tracking-widest rounded-lg"
                            >
                              Open in Google Maps <ExternalLink className="w-3 h-3 ml-1.5" />
                            </Button>
                          </div>
                        ) : (
                          msg.content
      ...
      <form onSubmit={sendMessage} className="p-6 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950/30">
        <div className="flex flex-col gap-3">
          {user?.role === 'seller' && listing?.seller?._id === user?.id && listing?.preciseLocation && (
            <Button
              type="button"
              onClick={shareLocation}
              disabled={sharingLocation}
              variant="outline"
              className="w-full rounded-xl border-emerald-500/20 bg-emerald-500/5 text-emerald-600 hover:bg-emerald-500/10 h-10 text-[10px] font-black uppercase tracking-widest mb-1"
            >
              {sharingLocation ? <Loader2 className="w-3 h-3 animate-spin mr-2" /> : <MapPin className="w-3 h-3 mr-2" />}
              Share Precise Collection Location
            </Button>
          )}
